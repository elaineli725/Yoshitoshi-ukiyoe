...
function WorkDetail({ works, index, showBackHome = true }) {
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 text-moon sm:px-6">
      ...
      <section className="mt-6 grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1fr)]">
        ...
        <article className="rounded-2xl border border-haze/20 bg-panel/35 p-6">
          <h1 className="font-serifCn text-3xl">{work.title}</h1>
...
function WorkSequencePage({ works }) {
  const { index: indexParam } = useParams();
  const navigate = useNavigate();
  const index = Number(indexParam);
  const isValidIndex = Number.isInteger(index) && index >= 1 && index <= works.length;
  ...
  }, [index, isValidIndex, navigate, works.length]);

  if (!isValidIndex) {
    return null;
  }

  const isLast = index === works.length;

  return (
    <main className="mx-auto max-w-6xl px-4 py-10 text-moon sm:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="font-serifCn text-3xl">作品展示（{index} / {works.length}）</h1>
        <Link to="/" className="inline-flex items-center rounded-full border border-haze/40 px-4 py-2 text-sm">
          ← 返回首页
        </Link>
      </div>

      <WorkDetail works={works} index={index} showBackHome={false} />

      <div className="mt-8 flex items-center justify-end gap-3">
        <button
          type="button"
          onClick={() => navigate(`/works/${Math.max(1, index - 1)}`)}
          disabled={index === 1}
          className="rounded-full border border-haze/40 px-5 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-40"
        >
          上一页
        </button>
        <button
          type="button"
          onClick={() => (isLast ? navigate('/closing') : navigate(`/works/${index + 1}`))}
          className="rounded-full border border-[#f0e5d6]/50 bg-[#181319]/70 px-7 py-3 text-base text-[#f6ecdf] shadow-[0_0_24px_rgba(255,255,255,0.18)]"
        >
          {isLast ? '进入结语' : '下一页'}
        </button>
      </div>

      {isLast ? <p className="mt-4 text-right text-sm text-moon/70">已到第 100 幅，页面将自动进入结语…</p> : null}
    </main>
  );
}
    ...
    <div className="bg-paper text-moon">
      <main className="flex min-h-screen items-center justify-center px-5 py-8">
        <motion.section
          ...
          initial="hidden"
          animate="show"
          className="relative aspect-[16/9] w-full max-w-[1920px] overflow-hidden rounded-3xl border border-haze/35 px-8 py-16 text-[#f1e8de] shadow-[0_28px_56px_rgba(20,16,18,0.36)] sm:px-14 lg:px-24"
          style={{
            backgroundImage: `linear-gradient(112deg, rgba(12, 12, 16, 0.86) 0%, rgba(33, 31, 38, 0.66) 46%, rgba(11, 10, 14, 0.86) 100%), url(${heroBackdropImage})`,
        ...
        >
          <div className="relative flex h-full max-w-3xl flex-col justify-center space-y-8">
            <p className="font-serifCn text-6xl leading-tight sm:text-7xl">月百姿</p>
            <p className="font-serifCn text-2xl text-[#f3e7d8]/90">月冈芳年笔下的月夜人间</p>
            <div className="flex flex-col items-start gap-4">
              <Link
                to="/works/1"
                className="rounded-full border-2 border-[#f3e7d8]/90 bg-[#151016]/82 px-10 py-4 text-2xl font-semibold tracking-wide text-[#fff7ea] shadow-[0_0_28px_rgba(255,240,212,0.52)] transition hover:scale-105 hover:shadow-[0_0_42px_rgba(255,240,212,0.72)]"
              >
                Begin Wandering
              </Link>
              <div className="mt-2 flex flex-wrap gap-3 text-sm sm:text-base">
                <Link to="/about" className="rounded-full border border-[#efe1cc]/65 bg-[#1e1820]/45 px-5 py-2 text-[#f2e8da]">
                  作品简介
                </Link>
...
function AboutPage() {
  return (
    <main className="mx-auto max-w-6xl px-4 py-14 text-moon sm:px-6">
      <Link to="/" className="inline-flex items-center rounded-full border border-haze/40 px-4 py-2 text-sm">
        ← 返回首页
      </Link>
      <section className="mt-8 grid gap-8 rounded-2xl border border-haze/20 bg-panel/45 p-8 lg:grid-cols-3">
        <h2 className="font-serifCn text-3xl">作品简介</h2>
...
function AuthorPage() {
  return (
    <main className="mx-auto max-w-6xl px-4 py-14 text-moon sm:px-6">
      ...
      <section className="mt-8 rounded-2xl border border-haze/20 bg-panel/45 p-8">
        <h2 className="font-serifCn text-3xl">作者介绍</h2>
...
function LegacyWorkDetailPage({ works }) {
  const { id } = useParams();
  const work = works.find((item) => item.id === id);
  const index = work ? Number(work.id.replace('w', '')) : 0;
  ...
  return <WorkDetail works={works} index={index} />;
}
...
export function App() {
  ...
  return (
    <Routes>
      ...
      <Route path="/works/:index" element={<WorkSequencePage works={works} />} />
      <Route path="/works" element={<WorksGalleryPage works={works} />} />
      <Route path="/works/id/:id" element={<LegacyWorkDetailPage works={works} />} />
    </Routes>
  );
}
